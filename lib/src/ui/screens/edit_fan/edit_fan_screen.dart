...
class EditFanScreen extends StatefulWidget {
  const EditFanScreen({super.key, this.fan});

  final FanModel? fan;

  @override
  State<EditFanScreen> createState() => _EditFanScreenState();
}
...
class _EditFanScreenState extends State<EditFanScreen> {
  ...
  void initState() {
    ...
    if (widget.fan != null) {
      _nameController.text = widget.fan!.name;
      _descriptionController.text = widget.fan!.description ?? 'No description';

      if (widget.fan!.image != null) {
        _image = File(widget.fan!.image!);
        _isImageAdded = true;
      }

      if (widget.fan!.audioFile != null) {
        _audioFile = File(widget.fan!.audioFile!);
        _isAudioAdded = true;
      }
    }
  }
  ...
  bool get isFilled => _nameController.text.isNotEmpty && _image != null;
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        floatingActionButton: AppButton(
          onPressed: () {
            final updatedFan = FanModel(
              name: _nameController.text,
              id: widget.fan!.id,
              image: _image?.path ?? widget.fan!.image,
              isCollection: widget.fan!.isCollection,
              isFavorite: widget.fan!.isFavorite,
              audioFile: _audioFile?.path ?? widget.fan!.audioFile,
              addedDate: widget.fan!.addedDate,
            );

            context.read<FanBloc>().add(AddFan(updatedFan));
